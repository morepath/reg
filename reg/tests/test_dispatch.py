...
def test_dispatch_register_value_single():
    @dispatch('obj')
    def foo(obj):
        pass
    ...
    foo.register_value(Bar, for_bar)
    foo.register_value(Qux, for_qux)

    assert foo(Bar()) == "bar"
    assert foo(Qux()) == "qux"


def test_dispatch_register_value_tuple_one_element():
    @dispatch('obj')
    def foo(obj):
        pass
...
def test_dispatch_register_value_tuple_multi():
    @dispatch('obj', 'second')
    def foo(obj, second):
        return "default"

    def for_bar_bar(obj, second):
        return "bar bar"

    def for_qux_qux(obj, second):
        return "qux qux"
    ...
    foo.register_value((Bar, Bar), for_bar_bar)
    foo.register_value((Qux, Qux), for_qux_qux)

    assert foo(Bar(), Bar()) == "bar bar"
    assert foo(Qux(), Qux()) == "qux qux"


def test_dispatch_register_value_list_multi():
    @dispatch('obj', 'second')
    def foo(obj, second):
        return "default"
...
def test_dispatch_register_value_list_single():
    @dispatch('obj')
    def foo(obj):
        return "default"
